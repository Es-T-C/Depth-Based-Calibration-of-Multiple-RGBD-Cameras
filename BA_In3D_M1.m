%% functions/BA_In3D_M1.m
function BA_In3D_M1(objectFolder,numPos,numCam,enable_BA2Dparams)
% BA_In3D_M1...   Bundle‐adjust intrinsics/extrinsics by minimizing 3D reprojection error Method1
%   BA_In3D_M1(objectFolder,numPos,numCam,enable_BA2Dparams)
...
%     enable_BA2Dparams: % 1 -> load BAin2D params, 0 -> create new initial parameter vector
    ...
    if enable_BA2Dparams==0
        ...
        filename = 'rot_matrices_BAin3D_M1.mat';
    ...
    save("params_BA_in3D_M1.mat","p");
    
    e1=errFun(par(:,1),x);
    E1=sqrt(e1'*e1)/(numPos*4*numPoint);
    
    e2=errFun(p,x);
    E2=sqrt(e2'*e2)/(numPos*4*numPoint);
    perc=(E1-E2)*100/E1;
    disp(['     ->Error on world coords. is reduced ' int2str(perc) '%'])
    fprintf('     ->Error with 2D refined parameters : %0.4e mm\n',E1)
    fprintf('     ->Error with 3D_M1 refined parameters : %0.4e mm\n',E2)
end
%%
...
camFolder = fullfile(objectFolder, sprintf('cam%d', 1));
load(fullfile(camFolder, 'closest_xpx_ypx_and_center_Z_selected68points.mat'))
load('rot_matrices_BAin3D_M1.mat','R1','R2','R3','R4')
pixelsize=0.0104;
numPoint=68;
...
function e = dxyz_function(R,x_mm, y_mm, z_mm, fx, fy, ox, oy, ~, ~, wx, wy, wz, tx, ty, tz, Xw, Yw, Zw)
